...
class Customer:
    def __init__(self, id, events):
        self.id = id
        self.events = events  # customer requests
        self.recvMsg = []
        self.channel = None
        self.stub = None
        self.port = 50000 + id
        self.lastProcessedId = -1
        self.clock = 1  # set initial logical clock as 0
    ...
    def executeEvents(self):
        ...
        res = {"id": self.id, "type": "customer", "events": []}
            ...
            if self.events[i]["interface"] == "deposit":
                self.stub.MsgDelivery(
                    branch_pb2.MsgDeliveryRequest(
                        id=self.id,
                        event_id=self.events[i]["customer-request-id"],
                        interface="deposit",
                ...
                res["events"].append(
                    {
                        "customer-request-id": self.events[i][
                            "customer-request-id"
                        ],
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=len(customers)
    ) as executor:
        customer_futures = [
            executor.submit(execute_customer, customer)
    ...
    for i in range(len(customer_data)):
        ...
        for j in range(len(customer_data[i]["events"])):
            all_events.append(
                {
                    "id": id,
                    "customer-request-id": customer_data[i]["events"][j][
                        "customer-request-id"
                    ],
                    "type": "customer",
                    "logical_clock": customer_data[i]["events"][j][
                        "logical_clock"
                    ],
            ...
            customer_request_id = customer_data[i]["events"][j][
                "customer-request-id"
            ]
            filtered_events = filter(
                lambda event: event["customer-request-id"]
                == customer_request_id,
                flattened_data,
            )
