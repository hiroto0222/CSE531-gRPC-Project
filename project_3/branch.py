...
class Branch(branch_pb2_grpc.BranchServicer):
    ...
    # Propagate Withdraw Definition
    def propagate_withdraw(self, amount):
        propagate_withdraw_request = RequestElement(
            id=-1, interface="propagate_withdraw", money=amount
        )
        request = MsgDeliveryRequest(
            request_elements=[propagate_withdraw_request]
        )
        for stub in self.stubList:
            stub.MsgDelivery(request)

    # Propagate Deposit Definition
    def propagate_deposit(self, amount):
        propagate_deposit_request = RequestElement(
            id=-1, interface="propagate_deposit", money=amount
        )
        request = MsgDeliveryRequest(
            request_elements=[propagate_deposit_request]
        )
        for stub in self.stubList:
            stub.MsgDelivery(request)

    # Handle Propagated Withdraw
    def handle_propagated_withdrawal(self, amount):
        self.balance -= amount
    ...
    # Handle Propagated Deposit
    def handle_propagated_deposit(self, amount):
        self.balance += amount
    ...
    def MsgDelivery(self, request, context):
        ...
        for request_element in request.request_elements:
            ...
            if interface == "query":
                message_received = self.handle_query()
            if interface == "withdraw":
                message_received = self.handle_withdraw(money)
            if interface == "deposit":
                message_received = self.handle_deposit(money)
...
def serve_branches(branch_id, port, branch_ports):
    port = port
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
    Branch(id=branch_id, balance=0, branches=branch_ports)
